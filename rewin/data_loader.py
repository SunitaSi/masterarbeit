...
def load_segmented_tssb_dataset():
    desc_filename = "datasets/TSSB/desc.txt"
    desc_file = []

    with open(desc_filename, 'r') as file:
        for line in file.readlines():
            desc_file.append(line.strip().split(","))

    df = []

    for row in desc_file:
        (ts_name, window_size), change_points = row[:2], row[2:]
        change_points = np.array([int(_) for _ in change_points])

        ts = np.loadtxt(fname=os.path.join('datasets/TSSB/', ts_name + '.txt'),
                        dtype=np.float64)

        # Append the last index for proper slicing
        segment_boundaries = np.concatenate(([0], change_points, [len(ts)]))

        for i in range(len(segment_boundaries) - 1):
            segment = ts[int(segment_boundaries[i]):int(segment_boundaries[i+1])]

            segment_name = f"{ts_name}{i+1}"
            df.append((segment_name, int(window_size), segment))

    return pd.DataFrame.from_records(df, columns=["name", "window_size", "time_series"])
